...
#import "AppHelper.h"
@interface E_ScrollViewController ()<UIPageViewControllerDataSource,UIPageViewControllerDelegate,E_ReaderViewControllerDelegate,E_SettingTopBarDelegate,E_SettingBottomBarDelegate,E_DrawerViewDelegate,E_SearchViewControllerDelegate>
{
...
{
    [super viewDidLoad];
//    CatalogText *acta=[CatalogText siged];
//    [CatalogText loadText:nil];
    
//    NSArray *array = [CatalogText getCatalog];
    //设置总章节数
    [E_ReaderDataSource shareInstance].totalChapter = [[CatalogText siged] NumBook];
...
- (void)turnToClickChapter:(NSInteger)chapterIndex{
    ...
    E_EveryChapter *chapter = [[E_ReaderDataSource shareInstance] openChapter:chapterIndex];//加1 是因为indexPath.row从0 开始的
    [self parseChapter:chapter];
...
- (void)turnToNextChapter{
    ...
    if ([E_ReaderDataSource shareInstance].currentChapterIndex == [E_ReaderDataSource shareInstance].totalChapter) {
        [AppHelper showTextOnly:@"已经是最后一章"];
        return;
    }
...
- (void)doMark{
    ...
    NSRange range = [_paginater rangeOfPage:_readPage];
    NSInteger bn = [E_ReaderDataSource shareInstance].currentChapterIndex;
    if (!bn) {
        bn = 0;
    }
    [E_CommonManager saveCurrentMark:bn andChapterRange:range byChapterContent:_paginater.contentText];
}
