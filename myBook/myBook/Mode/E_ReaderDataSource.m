...
//    
//    NSString *chapter_num = [NSString stringWithFormat:@"凡人修仙传"];
//    NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
//    NSString *path1 = [[NSBundle mainBundle] pathForResource:chapter_num ofType:@"txt"];
//    chapter.chapterContent = [NSString stringWithContentsOfFile:path1 encoding:enc error:NULL];
    ...
    NSString *bName = [NSString stringWithFormat:@"%@%ld.txt",[[CatalogText siged] getBookName],(long)clickChapter];
    NSString *filePath= [bookPath stringByAppendingPathComponent:bName];
    chapter.chapterContent = [NSMutableString stringWithContentsOfFile:filePath encoding:4 error:NULL];
//    chapter.chapterContent = [CatalogText getCatalog][clickChapter];
    return chapter;
}
...
//    NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
//    NSString *path1 = [[NSBundle mainBundle] pathForResource:chapter_num ofType:@"txt"];
//    NSString *str =[NSString stringWithContentsOfFile:path1 encoding:enc error:NULL];
...
- (E_EveryChapter *)nextChapter{
    ...
    if (_currentChapterIndex >= _totalChapter) {
        [AppHelper showTextOnly:@"没有更多内容了"];
        return nil;
...
- (E_EveryChapter *)preChapter{
    ...
    if (_currentChapterIndex <= 0) {
        [AppHelper showTextOnly:@"已经是第一页了"];
        return nil;
...
//    NSString *chapter_num = [CatalogText getCatalog][index];
//    NSString *path1 = [[NSBundle mainBundle] pathForResource:chapter_num ofType:@"txt"];
    NSString *content;
    if(index<[[CatalogText siged] NumBook]){
//        content =[CatalogText getCatalog][index];
        NSArray *cacesPath = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [cacesPath objectAtIndex:0];
        NSString *bookPath = [documentsDirectory stringByAppendingPathComponent:@"book"];
        NSString *bName = [NSString stringWithFormat:@"%@%ld.txt",[[CatalogText siged] getBookName],(long)index];
        NSString *filePath= [bookPath stringByAppendingPathComponent:bName];
        content = [NSMutableString stringWithContentsOfFile:filePath encoding:4 error:NULL];
    }
    return content;
}
